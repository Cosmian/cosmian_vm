...
pub async fn get_tpm_quote(
    ...
    context: Data<Mutex<Context>>,
) -> ResponseWithError<Json<TpmQuoteResponse>> {
    let mut context = context
        .lock()
        .map_err(|_| Error::UnexpectedError("TPM already in use".to_owned()))?;
    ...
    let ima_first_line = reader
        .lines()
        .next()
        .ok_or_else(|| Error::UnexpectedError("Event log is empty".to_owned()))??;
    let ima_entry = ImaEntry::try_from(ima_first_line.as_str())?;
    let pcr_slot = ima_entry.pcr;

    let (quote, signature, public_key) =
        tpm_get_quote(&mut context, &[pcr_slot as u8], Some(&quote_param.nonce))?;

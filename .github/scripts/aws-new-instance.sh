...
if [ "$TECHNO" = "tdx" ]; then
  ...
  if [ "$DISTRIB" = "ubuntu" ]; then
    # Ubuntu SEV
    AMI_BASE=$(aws ec2 describe-images --filters "Name=name,Values=ubuntu/images/hvm-ssd-gp3/ubuntu-noble-24.04-amd64-server-20260218" --query "Images[*].{ID:ImageId}" --output text)
  else
    # Redhat SEV
